	...
	//显示:当前日期
		...
		/*显示图片轮播的情况*/
		$(document).on("tap",".classroomBox ul.kthdjl li:nth-child(1)",function(){
		...
		//查看html引入
		$(document).on("tap",".classroomBox ul.kthdjl li:nth-child(2)",function(){
			$(".zyx_html").show();
		});
		
		/*查看课堂实录视屏*/
		$(document).on("tap",".classroomBox ul.kthdjl li:nth-child(3)",function(){
			$(".videoMask").show();
		});
		...
		/*关闭课堂实录视屏*/
		$(document).on("tap",".videoMask .hide,.zyx_html .hide",function(){
			$(".videoMask").hide();
			$(".zyx_html").hide();
		});
